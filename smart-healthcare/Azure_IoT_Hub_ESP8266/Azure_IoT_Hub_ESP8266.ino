...
//  az iot hub monitor-events --login 'HostName=I-o-T.azure-devices.net;SharedAccessKeyName=iothubowner;SharedAccessKey=7d/psPhy1fpSKt+pZaB2a9ij13J2rJFKiAIoTNS0KWk=' --device-id 1 
...
static char* getTelemetryPayload(float temperature, float humidity)
{
  ...
  temp_span = az_span_copy(temp_span, az_span_create_from_str(temp_str));

  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \"humidity\": "));
  
  char humidity_str[8];
  dtostrf(humidity, 5, 2, humidity_str);
  temp_span = az_span_copy(temp_span, az_span_create_from_str(humidity_str));
...
static void sendTelemetry(float temperature, float humidity)
{
